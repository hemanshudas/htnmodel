...
def complete_sim(target):
    ...
    def simulate_cohort(time_horizon, mi_fatal, stroke_fatal, int_composite, seed_x):
        ...
        c_MI = 952 * (1.03 ** 3)
        c_ST = 860 * (1.03 ** 3)
        c_postMI = (3.41 * (1.03 ** 3)) + numpy.random.choice([7.667, 15.334], 1, replace = True, p= [0.5, 0.5])[0]
        c_postST = (5.08 * (1.03 ** 3)) + numpy.random.choice([7.667, 15.334], 1, replace = True, p= [0.5, 0.5])[0]

        # AntiHTN treatment initiation
        c_consult = normal_gen(1.95, 0.27, alloc_iterate) #Prinja 2020
        ...
        # Disability weight of each state for GBD
        u_W = 0
    ...
    file_name = ("/Aspire_" + str(target[6]) + "_" + gender_name + "_Cov_" + str(target[1]) + "_Comp_" + str(
